...
function getItem(index,callback){
	contractInscription.methods.records((index)).call({from:account},function(error,result){
		...
		$("#time").html(time)
		contractInscription.methods.inscriptionIndex(result.inscription).call({from:account},function(error,result2){
...
function loadTransfers(){
	contractInscription.methods.inscriptionRecordsCount(name).call({from:account},function(error,counter){
...
function getTransferItem(index,callback){
	contractInscription.methods.inscriptionRecords(name,index).call({from:account},function(error,result){
		console.log(result)
		var time=formatDate(parseInt(result.time)*1000);
