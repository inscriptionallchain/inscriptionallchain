...
function init(){
	...
	contractMarket.methods.tokenIndex(tokenId).call({from:account},function(error,token){
		...
		contractMarket.methods.tokenInfo(tokenName).call({from:account},function(error,result){
			contractInscription.methods.inscriptionIndex(tokenName).call({from:account},function(error,nameIndex){
				contractInscription.methods.inscriptions(nameIndex).call({from:account},function(error,meta){
					...
					floorPrice=Math.round(result.floorPrice/10**18*limit * 1000000 ) / 1000000;
					...
					contractInscription.methods.inscriptionIndex(tokenName).call({from:account},function(error,tokenNum){
						contractInscription.methods.balanceOf(account,tokenNum).call({from:account},function(error,balance){
...
var ethRate=65;//代币汇率，根据汇率计算出价值多少U
function listed(){
	$("#listContainer").html('');
	contractMarket.methods.getSortedItems(tokenName).call({from:account},function(error,result){
		...
		for(var i=0;i<result.length;i++){
			...
			var uintPrice=Math.round(result[i].uintPrice*limit/10**18 * 10000000 ) / 10000000;
			...
			var div=`<div class="item">
			  <div class="cont1">
				...
				<div class="tit3">
					<img src="images/ics1.png" height="15" alt="">
			  ...
			  <div class="cont2">
				...
				<div class="infor2">
				  <p><img src="images/ics1.png" height="15" alt="">`+price+`ETH</p>
...
function activities(index){
	...
	contractMarket.methods.items(tokenName,index).call({from:account},function(error,result){
		...
		var uintPrice=Math.round(result.uintPrice*limit/10**18 * 10000000 ) / 10000000;
		...
		if(result.op=="1"){
			...
			op="<b style='color:rgb(214 57 42)'>Bought</b>";
		}else if(result.op=="3"){
		...
		var div=`<tr>
			...
			<td>
			  ...
			  <img src="images/ics1.png" height="15" alt="">
			  <span class="ver">`+uintPrice+`/Per Mint</span>
			</td>
			<td>
			  <div class="waptitle">
				Amount
...
function sumPrice(){
	...
	if(amount&&price){
		var inputPrice=Math.round(price * 1000000 ) / 1000000;
		var getPrice=Math.round(price*98/100 * 1000000 ) / 1000000;
		$("#inputPrice").html("$"+Math.round((inputPrice*ethRate) * 100 ) / 100+"per mint")
		$("#canGet").html(Math.round(getPrice*amount/limit * 1000 ) / 1000+"AVAX");
	}
}
...
function listToken(){
	...
	contractInscription.methods.allowance(tokenName,account,contracts.market.address).call({from:account},function(error,result){
		if(result>amount){
			var val=BigInt(Math.round(price*amount*10**18/limit))
			console.log("val",val)
