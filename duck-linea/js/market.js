...
function item(arr,index){
	...
	contractMarket.methods.tokenIndex(index).call({from:account},function(error,name){
		contractMarket.methods.tokenInfo(name).call({from:account},function(error,result){
			contractInscription.methods.inscriptionIndex(name).call({from:account},function(error,nameIndex){
				contractInscription.methods.inscriptions(nameIndex).call({from:account},function(error,meta){
					//console.log(result)
					limit=meta.limit;
					var floorPrice=Math.round(result.floorPrice/10**18*limit * 1000000 ) / 1000000;
